...
if exist "%ROOT_DIR%\build\bin\quiver_cli.exe" (
    ...
    REM --schema and --hub are mutually exclusive: must exit with code 2
    "%ROOT_DIR%\build\bin\quiver_cli.exe" --schema a.sql --hub m :memory: "%ROOT_DIR%\example\example1.lua" >nul 2>&1
    if !errorlevel! neq 2 (
        echo ERROR: CLI accepted --schema and --hub together
        SET CLI_RESULT=FAIL
        SET FAILED=1
    )
) else (

...
- `time_series_rows_from_lua` transpose, shared by `update_time_series_group_lua` and
  ...
  point's errors name it. The **dimension column(s) are the row-count authority**, discovered via
  public `get_time_series_metadata` (`dimension_column` plus any `value_columns` with
  `primary_key` set — the multi-dim case; `Database` exposes no Schema accessor so
  `internal::find_dimension_columns` is unreachable here). Dimension columns must be

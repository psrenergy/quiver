...
- Operator overloads in `quiver::` for boolean logic on nonzero-is-true operands: `operator&` / `operator|` (binary, same three combos) and `operator~` (unary). Produce `1.0`/`0.0`, **NaN propagates**, and the result is **unitless** — `&`/`|` skip unit-match validation (only shapes must broadcast) so conditions on different-unit variables compose; `~` emits a unitless result too. The bitwise-operator family is used because `and`/`or`/`not` are reserved keywords in C++/Lua (the NumPy/pandas convention); Lua binds the `__band`/`__bor`/`__bnot` metamethods on the Expression and BinaryFile usertypes, Julia overloads `Base.:&`/`:|`/`:~`.

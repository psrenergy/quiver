...
TimeFrequency frequency_from_string(const std::string& str) {
    if (str == "yearly") return TimeFrequency::Yearly;
    if (str == "monthly") return TimeFrequency::Monthly;
    if (str == "weekly") return TimeFrequency::Weekly;
    if (str == "daily") return TimeFrequency::Daily;
    if (str == "hourly") return TimeFrequency::Hourly;
...
int64_t TimeProperties::datetime_to_int(std::chrono::system_clock::time_point datetime) const {
    ...
    case TimeFrequency::Yearly:
        throw std::invalid_argument("YEARLY frequency extraction not implemented. This function should only be used for inner time dimensions.");
    case TimeFrequency::Monthly:
        return static_cast<unsigned>(ymd.month()); // 1-12
    case TimeFrequency::Weekly:
        throw std::invalid_argument("WEEKLY frequency extraction not implemented. This function should only be used for inner time dimensions.");
    case TimeFrequency::Daily:
...
std::chrono::system_clock::time_point TimeProperties::add_offset_from_int(std::chrono::system_clock::time_point base_datetime, int64_t value) const {

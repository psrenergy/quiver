...
static quiver::CSVExportOptions convert_export_options(const quiver_csv_export_options_t* opts) {
...
static quiver::CSVImportOptions convert_import_options(const quiver_csv_import_options_t* opts) {
    quiver::CSVImportOptions cpp_opts;
    cpp_opts.date_time_format = opts->date_time_format ? opts->date_time_format : "";
...
QUIVER_C_API quiver_csv_import_options_t quiver_csv_import_options_default(void) {
    quiver_csv_import_options_t opts = {};
    opts.date_time_format = "";
    return opts;
}
...
QUIVER_C_API quiver_error_t quiver_database_export_csv(quiver_database_t* db,
    ...
    try {
        auto cpp_opts = convert_export_options(opts);
...
QUIVER_C_API quiver_error_t quiver_database_import_csv(quiver_database_t* db,
                                                       ...
                                                       const char* group,
                                                       const char* path,
                                                       const quiver_csv_import_options_t* opts) {
    QUIVER_REQUIRE(db, collection, group, path, opts);

    try {
        auto cpp_opts = convert_import_options(opts);

...
protected:
    quiver::DatabaseOptions options{.read_only = false, .console_level = quiver::LogLevel::Off};
};
...
TEST_F(SchemaValidatorFixture, CollectionWithOptionalScalars) {
    ...
    // Read back - the NULL occupies its slot positionally (one entry per element).

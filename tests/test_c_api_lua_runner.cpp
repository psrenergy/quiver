...
TEST_F(LuaRunnerCApiTest, CreateElement) {
    ...
    size_t count = 0;
    auto read_result = margaux_read_scalar_integers(db, "Collection", "some_integer", &values, &count);
...
TEST_F(LuaRunnerCApiTest, ReuseRunner) {
    ...
    EXPECT_EQ(margaux_lua_runner_run(lua, R"(db:create_element("Configuration", { label = "Config" }))"), MARGAUX_OK);
    EXPECT_EQ(margaux_lua_runner_run(lua, R"(db:create_element("Collection", { label = "Item 1" }))"), MARGAUX_OK);
    EXPECT_EQ(margaux_lua_runner_run(lua, R"(db:create_element("Collection", { label = "Item 2" }))"), MARGAUX_OK);
...
TEST_F(LuaRunnerCApiTest, CreateElementWithVectors) {
    ...
    size_t count = 0;
    auto read_result = margaux_read_vector_integers(db, "Collection", "value_int", &vectors, &sizes, &count);
    EXPECT_EQ(read_result, MARGAUX_OK);
    EXPECT_EQ(count, 1);
    EXPECT_EQ(sizes[0], 3);
...
TEST_F(LuaRunnerCApiTest, UpdateElement) {
    ...
    int64_t value = 0;
    int has_value = 0;
    auto read_result = margaux_read_scalar_integers_by_id(db, "Collection", "some_integer", 1, &value, &has_value);
    EXPECT_EQ(read_result, MARGAUX_OK);

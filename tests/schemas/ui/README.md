...
`no_ui_dir`, which deliberately has no `ui/` at all. Every file is a hand-written miniature — none
...
the remaining eight by 01-02. This file is mirrored by `tests/test_database_ui_corpus.cpp`'s
`ReadmeNamesEveryFixture` (every directory below must appear here) and
`FixtureLiteralsArePinned` (the `## Rendered literals` table below must match the fixture files

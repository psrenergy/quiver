...
TEST_F(TempFileFixture, ErrorStrings) {
    EXPECT_STREQ(margaux_error_string(DECK_DATABASE_OK), "Success");
    EXPECT_STREQ(margaux_error_string(DECK_DATABASE_ERROR_INVALID_ARGUMENT), "Invalid argument");
    EXPECT_STREQ(margaux_error_string(DECK_DATABASE_ERROR_DATABASE), "Database error");
    EXPECT_STREQ(margaux_error_string(DECK_DATABASE_ERROR_MIGRATION), "Migration error");
    EXPECT_STREQ(margaux_error_string(DECK_DATABASE_ERROR_SCHEMA), "Schema validation error");
    EXPECT_STREQ(margaux_error_string(DECK_DATABASE_ERROR_CREATE_ELEMENT), "Failed to create element");
    EXPECT_STREQ(margaux_error_string(DECK_DATABASE_ERROR_NOT_FOUND), "Not found");
...
TEST_F(TempFileFixture, LogLevelDebug) {
    auto options = database_options_default();
    options.console_level = DECK_DATABASE_LOG_DEBUG;
    auto db = database_open(":memory:", &options);
...
TEST_F(TempFileFixture, LogLevelInfo) {
    auto options = database_options_default();
    options.console_level = DECK_DATABASE_LOG_INFO;
    auto db = database_open(":memory:", &options);
...
TEST_F(TempFileFixture, LogLevelWarn) {
    auto options = database_options_default();
    options.console_level = DECK_DATABASE_LOG_WARN;
    auto db = database_open(":memory:", &options);
...
TEST_F(TempFileFixture, LogLevelError) {
    auto options = database_options_default();
    options.console_level = DECK_DATABASE_LOG_ERROR;
    auto db = database_open(":memory:", &options);
...
TEST_F(TempFileFixture, DefaultOptions) {
    ...
    EXPECT_EQ(options.console_level, DECK_DATABASE_LOG_INFO);
}
...
TEST_F(TempFileFixture, SetScalarRelationNullDb) {
    ...
    EXPECT_EQ(err, DECK_DATABASE_ERROR_INVALID_ARGUMENT);
}
...
TEST_F(TempFileFixture, SetScalarRelationValid) {
    ...
    EXPECT_EQ(err, DECK_DATABASE_OK);

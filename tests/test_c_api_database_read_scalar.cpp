...
TEST(DatabaseCApi, ReadElementCountTracksCurrentRows) {
    auto options = quiver::test::quiet_options();
    quiver_database_t* db = nullptr;
    ASSERT_EQ(quiver_database_from_schema(":memory:", VALID_SCHEMA("collections.sql").c_str(), &options, &db),
              QUIVER_OK);
    ...
    int64_t count = -1;
    ASSERT_EQ(quiver_database_read_element_count(db, "Collection", &count), QUIVER_OK);
    EXPECT_EQ(count, 0);
...
TEST(DatabaseCApi, ReadElementCountNotFound) {
    auto options = quiver::test::quiet_options();
    quiver_database_t* db = nullptr;
    ASSERT_EQ(quiver_database_from_schema(":memory:", VALID_SCHEMA("collections.sql").c_str(), &options, &db),
              QUIVER_OK);
    ...
    int64_t count = -1;
    EXPECT_EQ(quiver_database_read_element_count(db, "Nope", &count), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Cannot read_element_count: collection not found: Nope");

    quiver_database_close(db);
}

TEST(DatabaseCApi, ReadElementCountNullArguments) {
    int64_t count = -1;
    EXPECT_EQ(quiver_database_read_element_count(nullptr, "Collection", &count), QUIVER_ERROR);
    ...
    EXPECT_EQ(quiver_database_read_element_count(db, nullptr, &count), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: collection");

    EXPECT_EQ(quiver_database_read_element_count(db, "Collection", nullptr), QUIVER_ERROR);

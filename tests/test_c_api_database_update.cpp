...
TEST(DatabaseCApi, UpdateVectorIntegers) {
    ...
    quiver_element_set_array_integer(e, "value_int", values1, 3, nullptr);
    int64_t id = 0;
    ...
    quiver_element_set_array_integer(update, "value_int", new_values, 4, nullptr);
...
TEST(DatabaseCApi, UpdateVectorFloats) {
    ...
    quiver_element_set_array_float(e, "value_float", values1, 3, nullptr);
    int64_t id = 0;
    quiver_database_create_element(db, "Collection", e, &id);
    EXPECT_EQ(quiver_element_destroy(e), QUIVER_OK);
    ...
    quiver_element_set_array_float(update, "value_float", new_values, 2, nullptr);
    auto err = quiver_database_update_element(db, "Collection", id, update);
    EXPECT_EQ(quiver_element_destroy(update), QUIVER_OK);
    EXPECT_EQ(err, QUIVER_OK);
...
TEST(DatabaseCApi, UpdateVectorToEmpty) {
    ...
    quiver_element_t* update = nullptr;
    ASSERT_EQ(quiver_element_create(&update), QUIVER_OK);
    quiver_element_set_array_integer(update, "value_int", nullptr, 0, nullptr);
    auto err = quiver_database_update_element(db, "Collection", id, update);
    EXPECT_EQ(quiver_element_destroy(update), QUIVER_OK);
    EXPECT_EQ(err, QUIVER_OK);
...
TEST(DatabaseCApi, UpdateSetStrings) {
    ...
    quiver_element_set_array_string(update, "tag", new_tags, 3, nullptr);
    auto err = quiver_database_update_element(db, "Collection", id, update);
    EXPECT_EQ(quiver_element_destroy(update), QUIVER_OK);
    EXPECT_EQ(err, QUIVER_OK);
...
TEST(DatabaseCApi, UpdateSetToEmpty) {
    ...
    quiver_element_t* update = nullptr;
    ASSERT_EQ(quiver_element_create(&update), QUIVER_OK);
    quiver_element_set_array_string(update, "tag", nullptr, 0, nullptr);
    auto err = quiver_database_update_element(db, "Collection", id, update);
    EXPECT_EQ(quiver_element_destroy(update), QUIVER_OK);
    EXPECT_EQ(err, QUIVER_OK);
...
TEST(DatabaseCApi, UpdateElementWithTimeSeries) {
    ...
    quiver_element_set_array_string(e, "date_time", dates1, 2, nullptr);
    double vals1[] = {1.0, 2.0};
    quiver_element_set_array_float(e, "value", vals1, 2, nullptr);
    int64_t id = 0;
    ...
    quiver_element_set_array_string(update, "date_time", dates2, 3, nullptr);
    double vals2[] = {10.0, 20.0, 30.0};
    quiver_element_set_array_float(update, "value", vals2, 3, nullptr);
    auto err = quiver_database_update_element(db, "Collection", id, update);
    EXPECT_EQ(quiver_element_destroy(update), QUIVER_OK);
    EXPECT_EQ(err, QUIVER_OK);
...
TEST(DatabaseCApi, UpdateElementVectorFkLabels) {
    ...
    quiver_element_set_array_string(child, "parent_ref", init_refs, 1, nullptr);
    int64_t child_id = 0;
    ASSERT_EQ(quiver_database_create_element(db, "Child", child, &child_id), QUIVER_OK);
    EXPECT_EQ(quiver_element_destroy(child), QUIVER_OK);
    ...
    quiver_element_set_array_string(update, "parent_ref", new_refs, 2, nullptr);
    auto err = quiver_database_update_element(db, "Child", child_id, update);
    EXPECT_EQ(quiver_element_destroy(update), QUIVER_OK);
    EXPECT_EQ(err, QUIVER_OK);
...
TEST(DatabaseCApi, UpdateElementSetFkLabels) {
    ...
    quiver_element_set_array_string(child, "mentor_id", init_mentors, 1, nullptr);
    int64_t child_id = 0;
    ASSERT_EQ(quiver_database_create_element(db, "Child", child, &child_id), QUIVER_OK);
    EXPECT_EQ(quiver_element_destroy(child), QUIVER_OK);
    ...
    quiver_element_set_array_string(update, "mentor_id", new_mentors, 1, nullptr);
    auto err = quiver_database_update_element(db, "Child", child_id, update);
    EXPECT_EQ(quiver_element_destroy(update), QUIVER_OK);
    EXPECT_EQ(err, QUIVER_OK);
...
TEST(DatabaseCApi, UpdateElementTimeSeriesFkLabels) {
    ...
    quiver_element_set_array_string(child, "date_time", init_dates, 1, nullptr);
    const char* init_sponsors[] = {"Parent 1"};
    quiver_element_set_array_string(child, "sponsor_id", init_sponsors, 1, nullptr);
    int64_t child_id = 0;
    ASSERT_EQ(quiver_database_create_element(db, "Child", child, &child_id), QUIVER_OK);
    EXPECT_EQ(quiver_element_destroy(child), QUIVER_OK);
    ...
    quiver_element_set_array_string(update, "date_time", new_dates, 2, nullptr);
    const char* new_sponsors[] = {"Parent 2", "Parent 1"};
    quiver_element_set_array_string(update, "sponsor_id", new_sponsors, 2, nullptr);
    auto err = quiver_database_update_element(db, "Child", child_id, update);
    EXPECT_EQ(quiver_element_destroy(update), QUIVER_OK);
    EXPECT_EQ(err, QUIVER_OK);
...
TEST(DatabaseCApi, UpdateElementAllFkTypesInOneCall) {
    ...
    quiver_element_set_array_string(update, "parent_ref", new_refs, 1, nullptr);
    const char* new_dates[] = {"2025-01-01"};
    quiver_element_set_array_string(update, "date_time", new_dates, 1, nullptr);
    const char* new_sponsors[] = {"Parent 2"};
    quiver_element_set_array_string(update, "sponsor_id", new_sponsors, 1, nullptr);
    auto err = quiver_database_update_element(db, "Child", child_id, update);
    EXPECT_EQ(quiver_element_destroy(update), QUIVER_OK);
    EXPECT_EQ(err, QUIVER_OK);
...
TEST(DatabaseCApi, UpdateVectorStringsHappyPath) {
    ...
    quiver_element_t* update = nullptr;
    ASSERT_EQ(quiver_element_create(&update), QUIVER_OK);
    quiver_element_set_array_string(update, "label_value", values, 2, nullptr);
    auto err = quiver_database_update_element(db, "AllTypes", id, update);
    EXPECT_EQ(quiver_element_destroy(update), QUIVER_OK);
    EXPECT_EQ(err, QUIVER_OK);
...
// ============================================================================
// Relation update tests (update_relation / update_relation_by_label)
// ============================================================================

namespace {

// Two Parents and one Child from relations.sql; ids come back through the out-params.
quiver_database_t* open_relation_db(int64_t* out_parent_a, int64_t* out_parent_b, int64_t* out_child) {
    auto options = quiver::test::quiet_options();
    quiver_database_t* db = nullptr;
    EXPECT_EQ(quiver_database_from_schema(":memory:", VALID_SCHEMA("relations.sql").c_str(), &options, &db), QUIVER_OK);
    const auto make = [&](const char* collection, const char* label, int64_t* out_id) {
        quiver_element_t* e = nullptr;
        EXPECT_EQ(quiver_element_create(&e), QUIVER_OK);
        quiver_element_set_string(e, "label", label);
        EXPECT_EQ(quiver_database_create_element(db, collection, e, out_id), QUIVER_OK);
        EXPECT_EQ(quiver_element_destroy(e), QUIVER_OK);
    };
    make("Parent", "Parent A", out_parent_a);
    make("Parent", "Parent B", out_parent_b);
    make("Child", "Child 1", out_child);
    return db;
}

// parent_id of the child, or nullopt when the relation is cleared.
...
TEST(DatabaseCApi, UpdateRelationSetsForeignKeyById) {
    ...
    ASSERT_EQ(quiver_database_update_relation(db, "Child", "Parent", "id", child, "Parent A"), QUIVER_OK);

    EXPECT_EQ(parent_of_child(db, child), parent_a);
    quiver_database_close(db);
}

TEST(DatabaseCApi, UpdateRelationByLabelResolvesSource) {
    int64_t parent_a = 0;
    int64_t parent_b = 0;
    int64_t child = 0;
    auto* db = open_relation_db(&parent_a, &parent_b, &child);
    ...
    EXPECT_EQ(parent_of_child(db, child), parent_b);
    quiver_database_close(db);
}

// NULL target_label is std::nullopt in the core, for both symbols.
TEST(DatabaseCApi, UpdateRelationClearsForeignKeyWithNullTargetLabel) {
    ...
    ASSERT_EQ(quiver_database_update_relation(db, "Child", "Parent", "id", child, "Parent A"), QUIVER_OK);
    ASSERT_EQ(quiver_database_update_relation(db, "Child", "Parent", "id", child, nullptr), QUIVER_OK);
    EXPECT_FALSE(parent_of_child(db, child).has_value());
...
TEST(DatabaseCApi, UpdateRelationByLabelNonExistent) {
    int64_t parent_a = 0;
    int64_t parent_b = 0;
    int64_t child = 0;
    auto* db = open_relation_db(&parent_a, &parent_b, &child);
...
TEST(DatabaseCApi, UpdateRelationNullArgumentsRejected) {
    ...
    EXPECT_EQ(quiver_database_update_relation(nullptr, "Child", "Parent", "id", child, "Parent A"), QUIVER_ERROR);
    EXPECT_EQ(quiver_database_update_relation(db, nullptr, "Parent", "id", child, "Parent A"), QUIVER_ERROR);

...
TEST(BinaryCApiMetadata, GetNumberOfTimeDimensions) {
    const char* toml = R"(
version = "1"
...
TEST(BinaryCApiMetadata, TomlRoundTrip) {
...
labels = ["plant_1", "plant_2"]
    ...
    quiver_binary_metadata_t* md = nullptr;
    ASSERT_EQ(quiver_binary_metadata_from_toml(toml_input, &md), QUIVER_OK);
    ASSERT_NE(md, nullptr);
    ...
    size_t dim_count = 0;
    EXPECT_EQ(quiver_binary_metadata_get_dimension_count(md, &dim_count), QUIVER_OK);
    EXPECT_EQ(dim_count, 2u);

    // Serialize back to TOML
    char* toml_output = nullptr;
    EXPECT_EQ(quiver_binary_metadata_to_toml(md, &toml_output), QUIVER_OK);
    ASSERT_NE(toml_output, nullptr);

    // Parse again and verify
    quiver_binary_metadata_t* md2 = nullptr;
    ASSERT_EQ(quiver_binary_metadata_from_toml(toml_output, &md2), QUIVER_OK);

    char* unit2 = nullptr;
    EXPECT_EQ(quiver_binary_metadata_get_unit(md2, &unit2), QUIVER_OK);
    EXPECT_STREQ(unit2, "MW");
    quiver_binary_metadata_free_string(unit2);

    quiver_binary_metadata_free_string(toml_output);
...
TEST(BinaryCApiMetadata, FromElement) {
...
TEST(BinaryCApiMetadata, NullArgsErrorMessages) {
    ...
    EXPECT_EQ(quiver_binary_metadata_get_unit(nullptr, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: md");

    EXPECT_EQ(quiver_binary_metadata_get_version(nullptr, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: md");

    EXPECT_EQ(quiver_binary_metadata_get_initial_datetime(nullptr, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: md");

    EXPECT_EQ(quiver_binary_metadata_get_number_of_time_dimensions(nullptr, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: md");

    EXPECT_EQ(quiver_binary_metadata_get_labels(nullptr, nullptr, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: md");

    EXPECT_EQ(quiver_binary_metadata_get_dimension_count(nullptr, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: md");

    EXPECT_EQ(quiver_binary_metadata_get_dimension(nullptr, 0, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: md");
}
...
TEST(BinaryCApiMetadata, NullArgsOnGetterOutParams) {
    ...
    EXPECT_EQ(quiver_binary_metadata_get_version(md, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: out");

    EXPECT_EQ(quiver_binary_metadata_get_initial_datetime(md, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: out");

    EXPECT_EQ(quiver_binary_metadata_get_number_of_time_dimensions(md, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: out");

    EXPECT_EQ(quiver_binary_metadata_get_labels(md, nullptr, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: out");

    EXPECT_EQ(quiver_binary_metadata_get_dimension_count(md, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: out");

    EXPECT_EQ(quiver_binary_metadata_get_dimension(md, 0, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: out");

    EXPECT_EQ(quiver_binary_metadata_to_toml(md, nullptr), QUIVER_ERROR);
...
TEST(BinaryCApiMetadata, InvalidToml) {
    quiver_binary_metadata_t* md = nullptr;
    EXPECT_EQ(quiver_binary_metadata_from_toml("not valid toml {{{", &md), QUIVER_ERROR);
...
TEST(BinaryCApiMetadata, FreeStringNull) {
    EXPECT_EQ(quiver_binary_metadata_free_string(nullptr), QUIVER_OK);
}

TEST(BinaryCApiMetadata, FreeStringArrayNull) {
    EXPECT_EQ(quiver_binary_metadata_free_string_array(nullptr, 0), QUIVER_OK);
}
...
TEST(BinaryCApiMetadata, FreeDimensionNull) {
    EXPECT_EQ(quiver_binary_metadata_free_dimension(nullptr), QUIVER_ERROR);
...
TEST(BinaryCApiMetadata, FromElementNullArgs) {
    EXPECT_EQ(quiver_binary_metadata_from_element(nullptr, nullptr), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: el");
}

TEST(BinaryCApiMetadata, FromElementMissingRequiredField) {
    quiver_element_t* el = nullptr;
    ASSERT_EQ(quiver_element_create(&el), QUIVER_OK);

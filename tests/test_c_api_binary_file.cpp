...
TEST_F(BinaryCApiFixture, WriteReadRoundTrip) {
    ...
    // Write
    {
        ...
        double data[] = {1.5, 2.5};
        EXPECT_EQ(quiver_binary_file_write(binary_file, dim_names, dim_values, 2, data, 2), QUIVER_OK);
...
TEST_F(BinaryCApiFixture, WriteReadMultiplePositions) {
...

    {
        ...
        quiver_binary_file_write(binary_file, dim_names, dims1, 2, data1, 2);

        int64_t dims2[] = {2, 2};
        double data2[] = {3.0, 4.0};
        quiver_binary_file_write(binary_file, dim_names, dims2, 2, data2, 2);
...

    {
        ...
        int64_t dims1[] = {1, 1};
        quiver_binary_file_read(binary_file, dim_names, dims1, 2, 0, &out_data, &out_count);
        EXPECT_DOUBLE_EQ(out_data[0], 1.0);
        quiver_binary_file_free_float_array(out_data);

        int64_t dims2[] = {2, 2};
        quiver_binary_file_read(binary_file, dim_names, dims2, 2, 0, &out_data, &out_count);
...
TEST_F(BinaryCApiFixture, GetFilePath) {
    ...
    quiver_binary_file_free_string(file_path);
...
TEST_F(BinaryCApiFixture, ReadNullBinary) {
    double* out_data = nullptr;
    size_t out_count = 0;
    EXPECT_EQ(quiver_binary_file_read(nullptr, nullptr, nullptr, 0, 0, &out_data, &out_count), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: binary_file");
}
...
TEST_F(BinaryCApiFixture, ReadNullOutParams) {
    ...
    EXPECT_EQ(quiver_binary_file_read(binary_file, dim_names, dim_values, 2, 0, nullptr, nullptr), QUIVER_ERROR);
...
TEST_F(BinaryCApiFixture, WriteNullBinary) {
    ...
    int64_t dim_values[] = {1, 1};
    double data[] = {1.0};
    EXPECT_EQ(quiver_binary_file_write(nullptr, dim_names, dim_values, 2, data, 1), QUIVER_ERROR);
    EXPECT_STREQ(quiver_get_last_error(), "Null argument: binary_file");
}
...
TEST_F(BinaryCApiFixture, WriteNullData) {
    ...
    const char* dim_names[] = {"row", "col"};
    int64_t dim_values[] = {1, 1};
    EXPECT_EQ(quiver_binary_file_write(binary_file, dim_names, dim_values, 2, nullptr, 2), QUIVER_ERROR);
...
TEST_F(BinaryCApiFixture, FreeFloatArrayNull) {
    EXPECT_EQ(quiver_binary_file_free_float_array(nullptr), QUIVER_OK);
}
...
TEST_F(BinaryCApiFixture, ReadAllowNulls) {
    ...
    EXPECT_EQ(quiver_binary_file_read(binary_file, dim_names, dim_values, 2, 0, &out_data, &out_count), QUIVER_ERROR);

    // With allow_nulls should succeed
    EXPECT_EQ(quiver_binary_file_read(binary_file, dim_names, dim_values, 2, 1, &out_data, &out_count), QUIVER_OK);
    ASSERT_EQ(out_count, 2u);

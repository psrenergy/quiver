...
It is possible to store time series data in your database. Time series in `Margaux` are very flexible. You can have missing values, and you can have sparse data. 
...
Time series in `Margaux` are very flexible. You can have missing values, and you can have sparse data. 
...
db = Margaux.from_schema(db_path, path_schema)

Margaux.create_element!(db, "Configuration"; label = "Toy Case", value1 = 1.0)
...
```julia
df = Margaux.read_time_series_table(
    db,
    "Resource",
    "some_vector1",
    "Resource 1",
...
```julia
values = Margaux.read_time_series_row(
    db,
    "Resource",
    "some_vector1",
    Float64;
...
```julia
Margaux.update_time_series_row!(
    db,
    "Resource",
    "some_vector3",
    "Resource 1",

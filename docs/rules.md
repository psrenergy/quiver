...
SQL schemas for the `QUIVERDatabase` framework should follow the conventions described in this document. Note that this is a tool for creating and developing some kinds of applications. Not all tools will need to use this framework.
...
It is advised to create new migrations using the functions from `QUIVERDatabase`. First you need to make sure that the migrations directory is registered 
by the function `QUIVERDatabase.set_migrations_folder` and after that you can create a new migration using the function `QUIVERDatabase.create_migration`. This function will create a new migration file with the name and version specified by the user. The migration file will contain a template for the migration.

### Running migrations

To run migrations you need to use the function `QUIVERDatabase.apply_migrations!`. There are various versions of this function, each one tailored to make something easier for the user.

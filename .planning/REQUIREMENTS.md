...
- [ ] **WRITE-02**: `db:write_csv(path, opts)` returns a writer handle; `w:write_row({...})` appends
      one row; `w:close()` finishes the file. Streaming only — there is deliberately no whole-file form
- [ ] **WRITE-03**: The writer accepts exactly two options — `separator` (a single character,
...
- [ ] **WRITE-04**: A path resolves against the directory containing the database file and cannot
...
- [ ] **WRITE-08**: `write_csv` truncates an existing target at open — stated behaviour, not a guard

### Cell formatting and RFC 4180

- [ ] **FMT-01**: A cell is quoted iff it contains the configured separator, `"`, CR or LF; an
      internal `"` is escaped by doubling it
- [ ] **FMT-02**: An empty cell that is the row's only cell is written quoted (`""`). Unquoted, that
...
- [ ] **FMT-03**: Every record is terminated, including the last, with LF. The stream opens in
      `std::ios::binary` so Windows text-mode translation cannot silently turn that LF into CRLF
- [ ] **FMT-04**: Numbers are written with `std::to_chars` shortest round-trip, reusing the existing
...
- [ ] **FMT-06**: A boolean writes as `1`/`0`; a `nil` writes as an empty cell; a table, function or
...
- [ ] **FMT-08**: A row's cell count comes from a single pass over integer keys, never `#t` — which
      is undefined on a table with a trailing hole, the shape every nullable read produces
- [ ] **FMT-09**: No BOM; the quote character is `"`; output is UTF-8. None of these is an option

### Lua binding

- [ ] **LUA-09**: The options table is decoded with the collect-entries-then-validate shape used by
      `read_csv_options_from_lua`, never a throw inside sol2's `for_each` (D-17)
- [ ] **LUA-10**: The sandboxed path resolves *before* the options table is decoded, matching
      `db:read_csv`'s established evaluation order (D-22)
- [ ] **LUA-11**: The handle is a sol2 usertype owned by `std::unique_ptr`, registered with
...
- [ ] **TEST-09**: A single-column file whose cells are `nil` or `""` round-trips with every row
...
- [ ] **DOC-05**: `LUA_DB_API_REFERENCE` documents `db:write_csv`, `w:write_row` and `w:close` — the
...
| WRITE-08 | Phase 4 | Pending |
| FMT-01 | Phase 4 | Pending |
| FMT-02 | Phase 4 | Pending |
| FMT-03 | Phase 4 | Pending |
| FMT-04 | Phase 4 | Pending |
| FMT-05 | Phase 4 | Pending |
| FMT-06 | Phase 4 | Pending |
| FMT-07 | Phase 5 | Pending |
| FMT-08 | Phase 4 | Pending |
| FMT-09 | Phase 4 | Pending |
| LUA-09 | Phase 4 | Pending |
| LUA-10 | Phase 4 | Pending |
| LUA-11 | Phase 4 | Pending |

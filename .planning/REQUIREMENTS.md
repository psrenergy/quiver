...
- [x] **CAPI-01**: C API supports N typed value columns for time series update (columnar parallel arrays: column_names[], column_types[], column_data[], column_count, row_count)
- [x] **CAPI-02**: C API supports N typed value columns for time series read (returns columnar typed arrays matching schema)
- [x] **CAPI-03**: Free function correctly deallocates variable-column read results (string columns: per-element cleanup; numeric columns: single delete[])
- [x] **CAPI-04**: Column types use existing quiver_data_type_t enum (INTEGER, FLOAT, STRING) with no new type definitions
- [x] **CAPI-05**: C API validates column names and types against schema metadata before processing, returning clear error messages on mismatch
...
- [x] **BIND-05**: Lua multi-column time series operations have test coverage (implementation already works via sol2)

...
**Shipped:** 2026-09-16 | **Complete:** 26/26, none dropped, none adjusted mid-flight.

Outcomes worth recording:

- **PARSE-02..07 needed no code.** Verified empirically before planning: csv-parser already handled
  the quoted separator, embedded newline, doubled quote, BOM, CRLF and ragged rows correctly through
  the existing reader. They were discharged as test-only requirements.
- **LUA-06 needed no code either.** The positional `header`/`rows` shape locked in phase 1 (D-01)
  already satisfies "no column unreachable, none shadowing" — a name-to-index map was explicitly
  rejected because it would have *created* the shadowing the requirement forbids.
- **TEST-05 ran three times** (phase 2 executor, phase 2 verifier, phase 3 release gate), each with
  an explicit `-DQUIVER_BUILD_TESTS=ON` configure, never the plain `release` preset, which sets it
  OFF and would report success while testing nothing.
- **Two live bugs were found and fixed outside any requirement**: an unwrapped `weakly_canonical`
  filesystem error reaching Lua across all nine file-touching operations, and a header-nil
  divergence between the two CSV entry points. Both are covered by mutation-checked regression tests.

> Archived verbatim from `.planning/REQUIREMENTS.md` at milestone completion. Deferred v2 items
> (CSV writing) carry forward to the next milestone's fresh requirements file.

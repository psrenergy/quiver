...
**Shipped:** 2026-09-17 | **Complete:** 29/29, none dropped, none adjusted mid-flight.

Outcomes worth recording:

- **The load-bearing claim was executed, not reasoned.** WRITE-06 rested entirely on "one
  `sol::state::collect_garbage()` at `run()`'s return flushes an unclosed writer." Lua 5.4's
  incremental collector can defer a finalizer to a later cycle, so that claim was settled by
  compiling a probe against this repo's own vendored sol2/lua build and running it across five
  reachability shapes — the same discipline v1.0 applied to `std::to_chars`, and the reason
  neither shipped on standards text alone.
- **TEST-11 was observed RED before the fix, and the byte count was measured.** The roadmap asserted
  the unflushed file was "zero bytes"; research rated that MEDIUM, because a `std::ofstream` that
  had already spilled its buffer would make a large-fixture test pass for the wrong reason. The
  RED run measured **0 bytes** on a deliberately tiny fixture — the claim is now verified rather
  than repeated.
- **FMT-07's width check deliberately did not go where it looks like it belongs.** It lives in the
  Lua layer, not `csv_write::Writer`: the row ordinal already existed there, and moving it into
  `Writer` would have meant retaining the header plus a duplicate row counter for no caller that
  exists. `Writer`'s public interface is unchanged by this milestone; `src/csv_write.cpp` received
  a comment-only edit.
- **Two stale assertions were corrected, not merely supplemented.** Root `CLAUDE.md` and
  `CHANGELOG.md` both still claimed "`db:write_csv` is not exposed" after Phase 4 shipped it.
  DOC-06 corrected both; a repo-wide grep for the claim now returns nothing outside `.planning/`.
- **`bindings/js/src/lua-api.ts` was pulled into DOC-06's scope by explicit decision**, though
  DOC-06's text names only three files. FMT-07's pad/throw and WRITE-06's flush are both behaviour
  a script author needs, and the sync build gate does *not* force it — Phase 5 binds no new `db:`
  name, and that gate only checks names.
- **The weak-test trap was not repeated for a third time.** Every correctness assertion in this
  milestone round-trips through `db:read_csv`; none greps the emitted file. `export_csv`'s 118
  export-side tests that never re-import remain the counter-example this project plans against.

> Archived verbatim from `.planning/REQUIREMENTS.md` at milestone completion. Deferred v2 items
> (`TOML-01/02`, `UNIFY-01..03`, `PERF-01/02`) carry forward to the next milestone's requirements.

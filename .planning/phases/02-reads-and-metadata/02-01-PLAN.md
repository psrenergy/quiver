...
must_haves:
  truths:
    ...
    - "read_scalar_relation returns list[str | None] (related element labels, not IDs), mapping NULL/empty to None"
    - "No ResourceWarning or memory errors are raised when reading from collections under -W error::ResourceWarning"
  artifacts:

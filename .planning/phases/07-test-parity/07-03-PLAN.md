...
Check `conftest.py` for existing fixtures. If there is an `all_types_db` fixture, use it. Otherwise, create the DB inline in each test or add an `all_types_db` fixture to conftest.py if the pattern supports it. Use `schema_path("all_types.sql")` or equivalent.

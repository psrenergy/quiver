...
must_haves:
  truths:
    ...
    - "read_vector_group_by_id returns list of row dicts for multi-column vector groups"
...
7. Add `read_vector_group_by_id` method. Reads each value column by type, transposes to row dicts. Does NOT include `vector_index` (following Julia/Dart precedent -- `vector_index` is not a value column in metadata and is not accessible via existing read APIs; rows are returned in order so index is implicit):
...
- **test_read_vector_group_by_id**: Using `collections_db`. Create element, update vectors: `value_int = [1, 2, 3]`, `value_float = [1.1, 2.2, 3.3]`. Call `read_vector_group_by_id("Collection", "values", id)`. Assert result is a list of 3 dicts. Assert `result[0] == {"value_int": 1, "value_float": 1.1}`. Assert `result[2] == {"value_int": 3, "value_float": 3.3}`.
...
- vector_index is NOT included in read_vector_group_by_id output (follows Julia/Dart precedent)

...
must_haves:
  truths:
    ...
    - "CSV-02 (import_csv) is deferred in REQUIREMENTS.md since C++ implementation is a no-op stub"
  artifacts:
    ...
    - path: "bindings/python/src/quiver/database.py"
      provides: "export_csv method and _marshal_csv_export_options helper"
...
<objective>
Bind the C API CSV export function to Python with full CSVExportOptions struct marshaling, and drop CSV-02 (import_csv) from scope since the C++ implementation is a confirmed no-op stub.

Purpose: Completes CSV export capability in the Python binding, the only FFI work in Phase 6.
Output: Working `export_csv` method with CSVExportOptions dataclass, CFFI declarations, marshaling helper, tests, and REQUIREMENTS.md updated.
</objective>
...
5. Update `database.py` import line to include `CSVExportOptions`:
...
6. Update `__init__.py` to export `CSVExportOptions`:
Add `CSVExportOptions` to the import line and `__all__` list.
  </action>
  <verify>
Run `python -c "from quiver import CSVExportOptions; print(CSVExportOptions())"` to confirm import works and defaults are correct.
  </verify>
  <done>CSVExportOptions dataclass is importable with correct defaults. export_csv method exists on Database. CFFI declarations parse without error. _marshal_csv_export_options follows keepalive pattern.</done>
</task>
...
3. Update `.planning/REQUIREMENTS.md`:
   - Change CSV-02 line from `- [ ] **CSV-02**: import_csv for table data import *(Note: verify C++ implementation exists before binding -- may be dropped)*` to `- [ ] **CSV-02**: ~~import_csv for table data import~~ *Deferred: C++ implementation is a no-op stub (empty body at `src/database_describe.cpp:91`). Will bind when C++ implements it.*`
   - Update traceability table: change CSV-02 status from `Pending` to `Deferred`
  </action>
  <verify>
Run `bindings/python/test/test.bat` or `python -m pytest bindings/python/tests/test_database_csv.py -v` to verify all CSV tests pass. Verify REQUIREMENTS.md shows CSV-02 as Deferred.
  ...
  <done>All CSV export tests pass covering default export, enum labels, date formatting, combined options, group export, and NULL handling. REQUIREMENTS.md updated to defer CSV-02 with explanation.</done>
</task>

</tasks>

<verification>
1. `python -m pytest bindings/python/tests/test_database_csv.py -v` -- all CSV export tests pass
...
3. REQUIREMENTS.md shows CSV-02 as Deferred with explanation
</verification>
...
- CSV-02 deferred in REQUIREMENTS.md
</success_criteria>

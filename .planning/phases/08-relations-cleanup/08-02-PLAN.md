...
must_haves:
  ...
  artifacts:
    - path: "bindings/python/tests/test_database_create.py"
      provides: "TestFKResolutionCreate class with 10 test methods"
    ...
    - path: "bindings/python/tests/test_database_metadata.py"
      provides: "FK metadata tests confirming is_foreign_key and references_collection"
      contains: "is_foreign_key"
  key_links:
...
Purpose: Achieve full FK test parity with Julia/Dart bindings. The 3 old relation tests are replaced with 10 create tests + 8 update tests + FK metadata verification, covering all FK column types (scalar, vector, set, time series) and error cases.
Output: Comprehensive FK resolution test classes in test_database_create.py and test_database_update.py, plus FK metadata assertions.
</objective>
...
<task type="auto">
  <name>Task 1: Add TestFKResolutionCreate class with 10 FK create tests</name>
...
3. `test_vector_fk_labels(self, relations_db)` -- Create Parent 1 and Parent 2, then Child with `parent_ref=["Parent 1", "Parent 2"]`. Read back via `read_vector_integers_by_id("Child", "parent_ref", child_id)`, assert equals `[1, 2]`.

4. `test_set_fk_labels(self, relations_db)` -- Create Parent 1 and Parent 2, then Child with `mentor_id=["Parent 1", "Parent 2"]`. Read back via `read_set_integers_by_id("Child", "mentor_id", child_id)`, assert `sorted(result) == [1, 2]`.
...
9. `test_non_fk_string_rejection_error(self, relations_db)` -- Create Configuration and Parent. Attempt to create Child with `parent_id=1` and `score=["not_a_label"]` (score is a non-FK integer set column in `Child_set_scores`). Assert raises `QuiverError`.
...
**Self-reference test** (from original relation tests -- keep the scenario per locked decision): Add as 11th test or fold self-reference into the scalar FK label test. Recommendation: add `test_self_reference_fk(self, relations_db)` -- create Child 1, then Child 2 with `sibling_id="Child 1"`. Read back `sibling_id` via `read_scalar_integer_by_id`, assert equals 1.
...
Note on time series test: The `relations.sql` schema's `Child_time_series_events` table has columns `date_time` (TEXT, dimension) and `sponsor_id` (INTEGER, FK to Parent). When creating a Child with time series data, set both `date_time` and `sponsor_id` as parallel arrays via Element.set().
  ...
  <verify>
    ...
    <manual>Verify TestFKResolutionCreate class has 10+ test methods, all passing</manual>
  </verify>
  <done>
    - TestFKResolutionCreate class exists in test_database_create.py with 10+ test methods
    - All FK resolution create scenarios pass: scalar label, scalar integer, vector, set, time series, all-in-one, no-FK passthrough, missing target error, non-FK rejection error, atomicity (no partial writes), self-reference
...
<task type="auto">
  ...
  <action>
**Part A: Update tests (test_database_update.py)**
...
**Part B: FK metadata verification (test_database_metadata.py)**

The existing `test_database_metadata.py` already has FK metadata tests in `TestGetScalarMetadata`:
- `test_get_scalar_metadata_foreign_key` -- verifies `parent_id` has `is_foreign_key=True`, `references_collection="Parent"`
- `test_get_scalar_metadata_self_reference_fk` -- verifies `sibling_id` has `is_foreign_key=True`, `references_collection="Child"`

These already satisfy the CONTEXT.md requirement for FK metadata tests. **No changes needed to test_database_metadata.py** unless the existing tests are insufficient. Verify by reading the file -- if the two FK metadata tests exist and pass, this part is complete.

If for any reason the FK metadata tests are missing or incomplete, add them following the existing pattern in that file.
  </action>
  <verify>
    <automated>cd C:/Development/DatabaseTmp/quiver && bindings/python/test/test.bat</automated>
    <manual>Verify TestFKResolutionUpdate class has 8 test methods, all passing. Verify FK metadata tests exist in test_database_metadata.py.</manual>
  </verify>
  <done>
    ...
    - FK metadata tests exist and pass (is_foreign_key, references_collection)
    - Tests mirror Julia/Dart FK test suite categories
...
2. `TestFKResolutionCreate` in test_database_create.py has 10+ test methods
3. `TestFKResolutionUpdate` in test_database_update.py has 8 test methods
4. FK metadata tests pass in test_database_metadata.py
5. Total Python test count increased by ~18 tests compared to pre-phase baseline
</verification>

...
<task type="auto">
  ...
  <verify>
    1. `grep -r "delete_element_by_id" src/ tests/ include/quiver/database.h` returns ONLY hits in: (a) lua_runner.cpp Lua-facing strings, (b) C API files under src/c/. No hits in database.h, database_delete.cpp, or test files.
    2. `grep -r "set_scalar_relation" src/ tests/ include/quiver/database.h` returns ONLY hits in lua_runner.cpp Lua-facing strings and C API files.
    3. `grep -r "export_to_csv\|import_from_csv" src/ tests/ include/quiver/database.h` returns ONLY hits in C API files.
    4. `grep -r "read_time_series_group_by_id" src/ tests/ include/quiver/database.h` returns ONLY hits in lua_runner.cpp Lua-facing strings/helper name and C API files.

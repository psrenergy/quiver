...
must_haves:
  truths:
    ...
    - "Running clang-tidy against the codebase produces zero errors (suppressions documented via NOLINT comments for intentional exceptions)"
...
Run clang-tidy file by file against the compile_commands.json to identify violations:
```bash
clang-tidy -p build src/database.cpp src/database_read.cpp src/database_update.cpp src/database_create.cpp src/database_delete.cpp src/database_metadata.cpp src/database_time_series.cpp src/database_query.cpp src/database_relations.cpp src/database_describe.cpp src/schema.cpp src/schema_validator.cpp src/type_validator.cpp src/element.cpp src/result.cpp src/lua_runner.cpp --header-filter="(include/quiver/|src/)"
```
...
Also run on C API files:
```bash
clang-tidy -p build src/c/database.cpp src/c/database_create.cpp src/c/database_read.cpp src/c/database_update.cpp src/c/database_metadata.cpp src/c/database_query.cpp src/c/database_time_series.cpp src/c/database_relations.cpp src/c/element.cpp src/c/lua_runner.cpp --header-filter="(include/quiver/|src/)"
```
...
**Important:** Do NOT modify any behavior-affecting code while fixing clang-tidy violations. Only style/convention changes and NOLINT annotations.
  ...
  <verify>
1. Run clang-tidy on all source files and confirm zero errors:
```bash
clang-tidy -p build [all source files] --header-filter="(include/quiver/|src/)" 2>&1 | grep -c "error:"
```
Should output 0.
...
```
  ...
  <done>
clang-tidy runs against the full codebase with zero errors. Any NOLINT suppressions are documented with reasons. All existing tests pass with zero regressions.
  </done>
</task>

...
class TestImportCSVSelfReferenceFKReImport:
    ...
    def test_self_reference_fk_reimport(self, relations_db: Database, tmp_path):
        ...
        sib1 = relations_db.query_integer("SELECT sibling_id FROM Child WHERE label = ?", parameters=["Child1"])
        assert sib1 is None

        sib2 = relations_db.query_integer("SELECT sibling_id FROM Child WHERE label = ?", parameters=["Child2"])
        child1_id = relations_db.query_integer("SELECT id FROM Child WHERE label = ?", parameters=["Child1"])
        ...
        child3_id = relations_db.query_integer("SELECT id FROM Child WHERE label = ?", parameters=["Child3"])
        sib3 = relations_db.query_integer("SELECT sibling_id FROM Child WHERE label = ?", parameters=["Child3"])
        assert sib3 == child3_id

        sib4 = relations_db.query_integer("SELECT sibling_id FROM Child WHERE label = ?", parameters=["Child4"])

...
# SAFE-02/SAFE-03 (Phase 2, plan 02-03/02-08): the load-time struct-size gate must ACTIVELY CALL
# the four native *_sizeof() accessors (CFFI ABI mode resolves lib.<name> via dlsym-on-demand, so
# a missing symbol only raises at the call, never at ffi.dlopen -- RESEARCH Pitfall 4) and the
...
# wired gate. test_gate_ran_and_checked_all_four_structs_in_order below is the one that would
# have caught it: it observes the evidence the *import* left behind.


def test_native_accessors_match_cdef_sizes() -> None:
    """The happy path: importing/loading the package does not raise, and all four sizes agree."""
    lib = get_lib()
...
def test_gate_ran_and_checked_all_four_structs_in_order() -> None:

  ...
  group('CSV Import', () {
    ...
    test('self-reference FK re-import', () {
      ...
      try {
        ...
        final sib1 = db.queryInteger('SELECT sibling_id FROM Child WHERE label = ?', ['Child1']);
        expect(sib1, isNull);

        final sib2 = db.queryInteger('SELECT sibling_id FROM Child WHERE label = ?', ['Child2']);
        final child1Id = db.queryInteger('SELECT id FROM Child WHERE label = ?', ['Child1']);
        ...
        final child3Id = db.queryInteger('SELECT id FROM Child WHERE label = ?', ['Child3']);
        final sib3 = db.queryInteger('SELECT sibling_id FROM Child WHERE label = ?', ['Child3']);
        expect(sib3, child3Id);

        final sib4 = db.queryInteger('SELECT sibling_id FROM Child WHERE label = ?', ['Child4']);

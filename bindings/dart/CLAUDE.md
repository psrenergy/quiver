...
- **The checked-in `bindings.dart` predates the pinned ffigen (20.1.1).** Regenerating today
  ...
  `quiver_database_update_element_by_label` and `quiver_database_update_vector_group_by_label`.
...
- **Marshaling idiom**: every method allocates through a `package:ffi` `Arena` and releases in
  ...
  (used by `updateTimeSeriesGroup`, `upsertTimeSeriesRow`, `updateVectorGroup`,
  `updateVectorGroupByLabel` and `updateSetGroup`); query parameters through `_marshalParams`.

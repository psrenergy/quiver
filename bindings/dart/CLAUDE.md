...
- **The checked-in `bindings.dart` predates the pinned ffigen (20.1.1).** Regenerating today
  ...
  `quiver_database_number_of_elements` after them. Take the generator upgrade as its own
  deliberate change (regenerate, then fix the enum call sites here and in hub) rather than as a
  side effect of adding a C function.

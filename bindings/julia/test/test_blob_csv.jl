...
@testset "Blob CSV" begin
    ...
    @testset "Data values match" begin
        ...
        try
            ...
            Quiver.write!(blob; data = [3.5, 4.5], row = 1, col = 2)
            Quiver.close!(blob)
    ...
    @testset "Float precision" begin
        ...
        try
            ...
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [1.23456789], row = 1)
            Quiver.close!(blob)
    ...
    @testset "csv_to_bin non-time round-trip" begin
        ...
        try
            ...
            v = Quiver.read(blob; row = 1, col = 1)
            @test v[1] ≈ 1.5
            @test v[2] ≈ 2.5
            v2 = Quiver.read(blob; row = 3, col = 2)
            @test v2[1] ≈ 11.5
            @test v2[2] ≈ 12.5
            Quiver.close!(blob)
        finally
    ...
    @testset "csv_to_bin aggregated datetime with hourly" begin
        ...
        try
            md = make_hourly_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [42.0], day = 1, hour = 1)
            Quiver.close!(blob)
            ...
            blob = Quiver.open_file(path; mode = :read)
            v = Quiver.read(blob; day = 1, hour = 1)
            @test v[1] ≈ 42.0
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
    ...
    @testset "csv_to_bin aggregated date without hourly" begin
        ...
        try
            md = make_time_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [10.0, 20.0], stage = 1, block = 1)
            Quiver.close!(blob)
            ...
            blob = Quiver.open_file(path; mode = :read)
            v = Quiver.read(blob; stage = 1, block = 1)
            @test v[1] ≈ 10.0
            @test v[2] ≈ 20.0
            Quiver.close!(blob)
        finally
    ...
    @testset "csv_to_bin non-aggregated time dimensions" begin
        ...
        try
            md = make_time_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [5.0, 6.0], stage = 1, block = 1)
            Quiver.close!(blob)
    ...
    @testset "Bin to CSV to bin without hourly" begin
        ...
        try
            md = make_time_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [100.0, 200.0], stage = 1, block = 1)
            Quiver.write!(blob; data = [300.0, 400.0], stage = 2, block = 15)
            Quiver.close!(blob)
            ...
            blob = Quiver.open_file(path; mode = :read)
            v1 = Quiver.read(blob; stage = 1, block = 1)
            @test v1[1] ≈ 100.0
            @test v1[2] ≈ 200.0
            v2 = Quiver.read(blob; stage = 2, block = 15)
            @test v2[1] ≈ 300.0
            @test v2[2] ≈ 400.0
            Quiver.close!(blob)
        finally
    ...
    @testset "Bin to CSV to bin with hourly" begin
        ...
        try
            md = make_hourly_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [77.0], day = 1, hour = 12)
            Quiver.write!(blob; data = [88.0], day = 3, hour = 24)
            Quiver.close!(blob)
            ...
            blob = Quiver.open_file(path; mode = :read)
            @test Quiver.read(blob; day = 1, hour = 12)[1] ≈ 77.0
            @test Quiver.read(blob; day = 3, hour = 24)[1] ≈ 88.0
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
        end
    ...
    @testset "CSV to bin to CSV with hourly" begin
        ...
        try
            md = make_hourly_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [1.0], day = 1, hour = 1)
            Quiver.close!(blob)
    ...
    @testset "Round-trip with null values" begin
        ...
        try
            ...
            blob = Quiver.open_file(path; mode = :read)
            v = Quiver.read(blob; row = 1, col = 1, allow_nulls = true)
            @test isnan(v[1])
            @test isnan(v[2])
            Quiver.close!(blob)
        finally
    ...
    @testset "Aggregated and non-aggregated produce same binary" begin
        ...
        try
            ...
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [1.0, 2.0], stage = 1, block = 1)
            Quiver.write!(blob; data = [3.0, 4.0], stage = 1, block = 5)
            Quiver.close!(blob)
            ...
            blob = Quiver.open_file(path; mode = :read)
            v1a = Quiver.read(blob; stage = 1, block = 1)
            v1b = Quiver.read(blob; stage = 1, block = 5)
            Quiver.close!(blob)
            ...
            blob = Quiver.open_file(path; mode = :read)
            v2a = Quiver.read(blob; stage = 1, block = 1)
            v2b = Quiver.read(blob; stage = 1, block = 5)
            Quiver.close!(blob)
    ...
    @testset "Round-trip mixed time and non-time" begin
        ...
        try
            ...
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [99.0], month = 1, scenario = 1, day = 1)
            Quiver.write!(blob; data = [77.0], month = 2, scenario = 2, day = 15)
            Quiver.close!(blob)
            ...
            blob = Quiver.open_file(path; mode = :read)
            @test Quiver.read(blob; month = 1, scenario = 1, day = 1)[1] ≈ 99.0
            @test Quiver.read(blob; month = 2, scenario = 2, day = 15)[1] ≈ 77.0
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
        end

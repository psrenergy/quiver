...
@testset "Delete" begin
    @testset "Element By ID" begin
        ...
        PSRDatabase.create_element!(db, "Configuration"; label = "Config 3")
        ...
        PSRDatabase.delete_element_by_id!(db, "Configuration", Int64(2))
    ...
    @testset "Element With Vector Data (CASCADE)" begin
        ...
        PSRDatabase.delete_element_by_id!(db, "Collection", Int64(1))
        ...
        values = PSRDatabase.read_vector_integers(db, "Collection", "value_int")
    ...
    @testset "Element With Set Data (CASCADE)" begin
        ...
        PSRDatabase.create_element!(db, "Collection"; label = "Item 1", tag = ["alpha", "beta"])
        PSRDatabase.create_element!(db, "Collection"; label = "Item 2", tag = ["gamma", "delta"])
        ...
        sets = PSRDatabase.read_set_strings(db, "Collection", "tag")
        @test length(sets) == 1
    ...
    @testset "Non-Existent Element (Idempotent)" begin
        ...
        PSRDatabase.delete_element_by_id!(db, "Configuration", Int64(999))
    ...
    @testset "Does Not Affect Other Elements" begin
        ...
        values = PSRDatabase.read_scalar_integers(db, "Configuration", "integer_attribute")

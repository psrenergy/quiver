...
@testset "Blob" begin
    ...
    @testset "Write and read single position" begin
        ...
        try
            md = make_simple_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [1.0, 2.0], row = 1, col = 1)
            Quiver.close!(blob)
    ...
    @testset "Write and read multiple positions" begin
        ...
        try
            ...
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [1.0, 2.0], row = 1, col = 1)
            Quiver.write!(blob; data = [3.0, 4.0], row = 2, col = 2)
            Quiver.write!(blob; data = [5.0, 6.0], row = 3, col = 1)
            Quiver.close!(blob)

            blob = Quiver.open_file(path; mode = :read)
            @test Quiver.read(blob; row = 1, col = 1)[1] ≈ 1.0
            @test Quiver.read(blob; row = 2, col = 2)[1] ≈ 3.0
            @test Quiver.read(blob; row = 3, col = 1)[1] ≈ 5.0
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
        end
    ...
    @testset "Write and read all positions" begin
        ...
        try
            ...
            for r in 1:3, c in 1:2
                Quiver.write!(blob; data = [Float64(counter), Float64(counter + 1)], row = r, col = c)
                counter += 2
            end
            Quiver.close!(blob)
            ...
            counter = 0
            for r in 1:3, c in 1:2
                data = Quiver.read(blob; row = r, col = c)
    ...
    @testset "Overwrite existing data" begin
        ...
        try
            ...
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [1.0, 2.0], row = 1, col = 1)
            Quiver.write!(blob; data = [99.0, 100.0], row = 1, col = 1)
            Quiver.close!(blob)
    ...
    @testset "Negative values" begin
        ...
        try
            md = make_simple_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [-1.5, -999.99], row = 1, col = 1)
            Quiver.close!(blob)
    ...
    @testset "Zero values" begin
        ...
        try
            md = make_simple_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [0.0, 0.0], row = 1, col = 1)
            Quiver.close!(blob)
    ...
    @testset "Large double values" begin
        ...
        try
            ...
            big = 1e300
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [big, -big], row = 1, col = 1)
            Quiver.close!(blob)
    ...
    @testset "Explicit NaN write" begin
        ...
        try
            md = make_simple_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [NaN, NaN], row = 1, col = 1)
            Quiver.close!(blob)
    ...
    @testset "Wrong dimension count too few" begin
        ...
        try
            md = make_simple_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            @test_throws Quiver.DatabaseException Quiver.read(blob; row = 1, allow_nulls = true)
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
        end
    ...
    @testset "Wrong dimension count too many" begin
        ...
        try
            md = make_simple_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            @test_throws Quiver.DatabaseException Quiver.read(blob; row = 1, col = 1, z = 1, allow_nulls = true)
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
        end
    ...
    @testset "Missing dimension name" begin
        ...
        try
            md = make_simple_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            @test_throws Quiver.DatabaseException Quiver.read(blob; row = 1, bad = 1, allow_nulls = true)
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
        end
    ...
    @testset "Value below one" begin
        ...
        try
            md = make_simple_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            @test_throws Quiver.DatabaseException Quiver.read(blob; row = 0, col = 1, allow_nulls = true)
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
        end
    ...
    @testset "Value above max" begin
        ...
        try
            md = make_simple_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            @test_throws Quiver.DatabaseException Quiver.read(blob; row = 4, col = 1, allow_nulls = true)
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
        end
    ...
    @testset "Boundary max valid" begin
        ...
        try
            md = make_simple_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            data = Quiver.read(blob; row = 3, col = 2, allow_nulls = true)
            @test length(data) == 2
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
    ...
    @testset "Data too short" begin
        ...
        try
            md = make_simple_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            @test_throws Quiver.DatabaseException Quiver.write!(blob; data = [1.0], row = 1, col = 1)
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
        end
    ...
    @testset "Data too long" begin
        ...
        try
            md = make_simple_metadata()
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            @test_throws Quiver.DatabaseException Quiver.write!(blob; data = [1.0, 2.0, 3.0], row = 1, col = 1)
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
        end
    ...
    @testset "Valid time dimension coordinates" begin
        ...
        try
            ...
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [1.0, 2.0], stage = 1, block = 1)
            Quiver.close!(blob)
            @test true
        finally
            cleanup_blob(path)
    ...
    @testset "Invalid time dimension coordinates" begin
        ...
        try
            ...
            @test_throws Quiver.DatabaseException Quiver.write!(blob; data = [1.0, 2.0], stage = 2, block = 30)
            Quiver.close!(blob)
        finally
            cleanup_blob(path)
        end
    ...
    @testset "Single time dimension skips consistency check" begin
        ...
        try
            ...
            blob = Quiver.open_file(path; mode = :write, metadata = md)
            Quiver.write!(blob; data = [1.0], month = 12, scenario = 3)
            Quiver.close!(blob)
            @test true
        finally
            cleanup_blob(path)

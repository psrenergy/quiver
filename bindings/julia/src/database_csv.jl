...
"""
    build_quiver_csv_import_options(kwargs...) -> (Ref{quiver_csv_import_options_t}, Vector{Any})

Build a C API `quiver_csv_import_options_t` from keyword arguments.
Returns `(opts_ref, temps)` where `temps` holds all temporary objects that must
stay alive during the C call (use inside `GC.@preserve`).
...
"""
function build_quiver_csv_import_options(; kwargs...)
    opts = C.quiver_csv_import_options_default()
    temps = Any[]
    ...
    opts_new = C.quiver_csv_import_options_t(
        dtf_ptr,
        attr_names_ptr,
        locale_names_ptr,
        entry_counts_ptr,
...
function import_csv(db::Database, collection::String, group::String, path::String; kwargs...)
    opts_ref, temps = build_quiver_csv_import_options(; kwargs...)
    GC.@preserve temps begin

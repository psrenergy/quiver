mutable struct LuaRunner
    ptr::Ptr{C.margaux_lua_runner}
    db::Database  # Keep reference to prevent GC
end

function LuaRunner(db::Database)
    ptr = C.margaux_lua_runner_new(db.ptr)
    if ptr == C_NULL
        throw(DatabaseException("Failed to create LuaRunner"))
    end
    runner = LuaRunner(ptr, db)
    finalizer(r -> r.ptr != C_NULL && C.margaux_lua_runner_free(r.ptr), runner)
    return runner
end

function run!(runner::LuaRunner, script::String)
    err = C.margaux_lua_runner_run(runner.ptr, script)
    if err != C.MARGAUX_OK
        error_ptr = C.margaux_lua_runner_get_error(runner.ptr)
        if error_ptr != C_NULL
            error_msg = unsafe_string(error_ptr)
            throw(DatabaseException("Lua error: $error_msg"))
        else
            throw(DatabaseException("Lua script execution failed"))
        end
    end
    return nothing
end

function close!(runner::LuaRunner)
    if runner.ptr != C_NULL
        C.margaux_lua_runner_free(runner.ptr)
        runner.ptr = C_NULL
    end
    return nothing
end

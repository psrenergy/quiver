...
- **One marshaller for all three group writers**: `_update_group_columns(db, update_by_id,
  update_by_label, ...)` (`src/database_update.jl`) takes **both** C entry points and picks between
  them by `element isa Int64`, so `update_time_series_group!`, `update_vector_group!` and
  `update_set_group!` are one wrapper each taking `element::Union{Int64, String}` (no `_by_label`
  names). `upsert_time_series_row!` shares the same shape via `_upsert_time_series_row`. Don't copy
  the `GC.@preserve` body per writer, and **don't move the id/label choice back to the call site**:
  Julia converts an Integer to `Ptr{Cchar}` silently, so a mis-paired id reaches the C API as a
  pointer to address `<id>` and `std::string(label)` reads arbitrary memory. (The reverse
  mis-pairing fails cleanly with a `MethodError`, so only that one direction was dangerous.)

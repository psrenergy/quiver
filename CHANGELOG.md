...
- **`read_element_count(collection)`.** Returns the current number of rows in a
  ...
  gaps. The C API symbol is `quiver_database_read_element_count` and writes an `int64_t` scalar to
  caller-owned storage.

  Available in **every layer**: C++, the C API, Julia (`read_element_count`), Dart
  (`readElementCount`), Python (`read_element_count`), JS (`readElementCount`), and Lua
  (`db:read_element_count`). Every binding calls the scalar C entry point directly, so the count
